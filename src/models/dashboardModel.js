...
function pegarKPISensorproblema(idEmpresa) {
  var sql = `
    SELECT * FROM vw_KPISensorProblema;
  `;
...
function pegarKPIAlertasCriticos(idEmpresa) {
  var sql = `
  select * FROM vw_KPIAlertasCriticos where fk_id_empresa = ${idEmpresa}
  `;
  console.log("Executando a seguinte instrução SQL: \n", sql);
  return database.executar(sql);
}
...
function KPIMaiorPropCriticos(idempresa) {
  var sql = `
    select * FROM vw_MaiorPropCriticos WHERE fk_id_empresa = ${idempresa};
  `;
  console.log("Executando a seguinte instrução SQL: \n", sql);
  return database.executar(sql);
}
...
function ranking(idempresa) {
  var sql = `
    select * FROM vw_ranking where fk_id_empresa = ${idempresa};
  `;
  console.log("Executando a seguinte instrução SQL: \n", sql);
  return database.executar(sql);
}
